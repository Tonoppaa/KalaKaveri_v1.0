...
{
    public partial class admin_käyttäjähallinta_poista_käyttäjä : Form
    {
        string userID;
        MySqlConnection yhteys;
        public admin_käyttäjähallinta_poista_käyttäjä(MySqlConnection yhteysOlio, string uID)
        {
            InitializeComponent();
            userID = uID;
            yhteys = yhteysOlio;
        ...
        private void HaeKäyttäjäID() // Metodi, joka hakee käyttäjän käyttäjäID perusteella
        {
            try
            {
                yhteys.Open();
                {
                    if (!string.IsNullOrEmpty(kayttajaIDtextBox.Text)) // Tarkistetaan, onko kenttä tyhjä
                    {
                        ...
                        if (käyttäjäTable.Rows.Count > 0) // Tarkistetaan, onko rivien määriä 1 tai enemmän
                        {
        ...
        private void tyhjennäTaulubutton_Click(object sender, EventArgs e) // Tyhjennä datagridView
        {
        ...
        private void suljeNappi_Click(object sender, EventArgs e) // Sulje form ja avaa käyttäjänhallinta-form
        {
            this.Close();
        ...
        private void haeKokonimibutton_Click(object sender, EventArgs e) // Nappia painamalla suoritetaan HaeKokonimi-metodi, joka hakee käyttäjän kokonimen perusteella
        {
            HaeKokonimi();
        }

        private void tyhjennäKentätbutton_Click(object sender, EventArgs e) // Tyhjentää kaikki tekstikentät
        {
        ...
        private void käyttäjädataGridView_CellDoubleClick(object sender, DataGridViewCellEventArgs e) // Tuplaklikkaamalla datagridviewin riviä avataan uusi ikkuna, jossa käyttäjän tiedot näkyvät
        {
            if (e.RowIndex >= 0) // Tarkistetaan, että rivi on valittu
            {
                ...
                admin_käyttäjähallinta_poista_poistakäyttäjä poistakäyttäjä = new admin_käyttäjähallinta_poista_poistakäyttäjä(kayttajaID, etunimi, sukunimi, osoite, email, salasana, rooli, yhteys, userID);
                poistakäyttäjä.Show();
                this.Close();
            }
        }

        private void haeKaikkiKäyttäjätbutton_Click(object sender, EventArgs e) // Lataa kaikki käyttäjät datagridView:n
        {
            try
            {
                yhteys.Open();
