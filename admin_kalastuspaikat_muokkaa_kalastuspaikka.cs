...
{
    public partial class admin_kalastuspaikat_muokkaa_kalastuspaikka : Form
    {
        string userID;
        MySqlConnection yhteys;
        public admin_kalastuspaikat_muokkaa_kalastuspaikka(MySqlConnection yhteysOlio, string uID)
        {
            InitializeComponent();
            userID = uID;
            yhteys = yhteysOlio;
        ...
        private void LataaVesistöt() // Kun hae-nappia klikataan, haetaan tietokannasta tiedot datagridView:n
        {
            try
            {
                yhteys.Open();
                {
                    string haeVesistot = "SELECT vesistö AS Vesistö, paikkaID AS PaikkaID, kaupunki, koordinaatit AS Koordinaatit," +
        ...
        private void LataaKalapaikat() // Ladataan kalapaikat (kaupungit) comboboxiin
        {
            try
            {
                ...
                kalapaikatcomboBox.Items.Clear(); // Tyhjennetään combobox ennen uuden datan lataamista
        ...
        private void kalapaikatdatagridView_CellDoubleClick(object sender, DataGridViewCellEventArgs e) // Kun solua tuplaklikataan, avautuu kalastuspaikka form
        {
            if (e.RowIndex >= 0)
            {
                ...
                admin_kalastuspaikat_muokkaa_muokkaakalastuspaikka admin_muokkaakalastuspaikka = new admin_kalastuspaikat_muokkaa_muokkaakalastuspaikka
        ...
        private void haeNappi_Click(object sender, EventArgs e) // Kun hae-nappia klikataan, LataaVesistöt-metodi suoritetaan (lataa vesistöt datagridView)
        {
            LataaVesistöt();
        }
    }
